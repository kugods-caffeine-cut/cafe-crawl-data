...
for loop in range(131, 132): 
    ...
    with open((cwd+"/data/data_starbucks.json"),"r",encoding='utf-8') as file:
        JsonFile = json.load(file)
        JsonFile["count"] += 1
        JsonFile["item"].append(item)
    with open((cwd+"/data/data_starbucks.json"),"w",encoding='utf-8') as file:
